...
def init_train_state(
    ...
    # Wrap optimizer with gradient accumulation if configured.
    if config.grad_accumulation_steps > 1:
        tx = optax.MultiSteps(tx, every_k_schedule=config.grad_accumulation_steps)
        logging.info(f"Gradient accumulation enabled: {config.grad_accumulation_steps} steps (effective batch = {config.batch_size * config.grad_accumulation_steps})")
